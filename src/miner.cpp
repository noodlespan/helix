...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to ztcashs that have been in mempool long
                    //and higher priority to ztcashs that are large in value
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zTcash spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
